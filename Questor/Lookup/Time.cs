...
namespace Questor.Modules.Lookup
{
    public class Time
    {
        ...
        public int DelayBetweenJetcans_seconds = 185;
            // Once you have made a JetCan you cannot make another for 3 minutes, units: seconds. Default is 185 (to account for lag)
        ...
        public int FittingWindowLoadFittingDelay_seconds = 5;
            // We can ask the fitting to be loaded using the fitting window, but we cant know it is done, thus this delay, units: seconds. Default is 10
        ...
        public int NoGateFoundRetryDelay_seconds = 30;
        ...
        public int QuestorBeforeLoginPulseDelay_milliseconds = 500;
            // Pulse Delay for Program.cs: Used to control the speed at which the program will retry logging in and retry checking the schedule
        ...
        public int ScheduleCheck_seconds = 120;
            // How often when in IDLE, we should check to see if we need to logoff / restart, this can be set to a low number, default is 120 seconds (2 minutes)
        ...
        public int TravelerInWarpedNextCommandDelay_seconds = 15;
            // Traveler is in warp - delay before processing another command, units: seconds. Default is 15

        public int TravelerJumpedGateNextCommandDelay_seconds = 15;
            // Traveler jumped a gate - delay before assuming we have loaded grid, units: seconds. Default is 15

        public int TravelerNoStargatesFoundRetryDelay_seconds = 15;
            // Traveler could not find any StarGates, retry when this time has elapsed, units: seconds. Default is 15

        public int ValidateSettings_seconds = 15;
            // This is the delay between character settings validation attempts. The settings will be reloaded at this interval if they have changed. Default is 15
        ...
        public int WrecksDisappearAfter_minutes = 110;
            // used to determine how long a wreck will be in space: usually to delay salvaging until a later time, units: minutes. Default is 120 minutes (2 hours)
