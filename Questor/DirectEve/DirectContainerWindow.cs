...
namespace DirectEve
{
	...
	public class DirectContainerWindow : DirectWindow
	{
		...
		public bool IsReady
		{
			get { return !((bool) PyWindow.Attribute("startingup")) && !((bool) PyWindow.Attribute("loadingInvCont")) && !((bool) PyWindow.Attribute("loadingTreeView")); }
		}
		...
		public long GetEstimatedValue() {
			...
			long ret = 0;
			if(PyWindow.IsValid) {
				var totalPriceLabelText = PyWindow.Attribute("totalPriceLabel").Attribute("text").ToUnicodeString();
				Console.WriteLine("Label value: " + totalPriceLabelText);
			}
			return ret;
		}
		...
		{
			return Type == "form.InventoryPrimary";
		}
	}
}