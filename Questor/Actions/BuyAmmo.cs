			...
			switch (state)
			{
				...
				case BuyAmmoState.BuyAmmo:
					...
					if (currentAmmoDirectItem != null)
					{
						...
						if (avgPrice != 0)
						{
							maxPrice = avgPrice * 3; // 3 times the avg price
						}
						else
						{
							if(basePrice != 0)
							{
								maxPrice = basePrice * 6; // 6 times the base price
							} else {
								
								maxPrice = 1000; // if everything else falls through we limit the price to 1000 isk to not get fooled by market manipulations, this also passivly disables buying drones
							}
						}
