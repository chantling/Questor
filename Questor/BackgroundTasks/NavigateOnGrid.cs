			...
			if (DateTime.UtcNow > Time.Instance.NextOrbit)
			{
				...
				if (target.Distance + OrbitDistanceToUse < Combat.MaxRange - 5000)
				{
					...
					if (!Cache.Instance.IsApproachingOrOrbiting(target.Id))
					{
						...
						{
							if (Combat.DoWeCurrentlyHaveTurretsMounted() && target.IsFrigate)
							{
								Logging.Log(module, "Target is a frigate and we have turrets mounted, using three times the usual orbit distance.", Logging.Teal);
