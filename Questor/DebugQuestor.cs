...
namespace Questor.Modules
{
    ...
    public class DebugQuestor
    {
        ...
        private void LootHostileWrecks()
        {
            ...
            foreach (var window in lootWindows)
            {
                ...
                foreach (var item in items) //.OrderByDescending(i => i.IskPerM3))
                {
                    ...
                    if ((freeCargoCapacity - item.TotalVolume) <= (isMissionItem ? 0 : ReserveCargoCapacity))
                    {
                        ...
                        {
                            if (isMissionItem)
                                Logging.Log("Scoop: Not enough space for [" + item.Name + "] Need [" + item.TotalVolume + "]m3 - maximum available [" + (freeCargoCapacity + worthLess.Sum(wl => wl.TotalVolume)) + "]m3");
                        ...
                        if (moveTheseItems.Count > 0)
                        {
                            ...
                            Logging.Log("Scoop: Moving [" + moveTheseItems.Count + "] items into the cargo container to make room for the more valuable loot");
                        }
                    }
            ...
            foreach (var containerEntity in Cache.Instance.UnlootedWrecksAndSecureCans.Where(e => e.Distance <= 2500))
            {
                // Emptry wreck, ignore
