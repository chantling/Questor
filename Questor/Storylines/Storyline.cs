		...
		public void ProcessState()
		{
			switch (_States.CurrentStorylineState)
			{
				...
				case StorylineState.BlacklistAgent:
					...
					Logging.Log("Storyline", "BlacklistAgent: The agent that provided us with this storyline mission has been added to the session blacklist AgentId[" +  Cache.Instance.CurrentStorylineAgentId + "]", Logging.White);
					
					
					var currentStorylines = Cache.Instance.DirectEve.AgentMissions.Where(m => m.AgentId == Cache.Instance.CurrentStorylineAgentId).Where(m => m.Type.Contains("Storyline") && m.State == (int)MissionState.Offered).ToList();
