        ...
        private bool WeShouldBeInSpaceORInStationAndOutOfSessionChange()
        {
            if (!Cache.Instance.InSpace && !Cache.Instance.InStation)
            {
                if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange)
                    Logging.Log("Debug3", "if (!Cache.Instance.InSpace && !Cache.Instance.InStation)", Logging.Debug);
            ...
            {
                if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange)
                    Logging.Log("Debug3", "InSpace or InStation and still in a session change", Logging.Debug);
                //wait... we must be in a session change
                return false;
            }

            if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange)
                Logging.Log("Debug3", "We are not in a session change, continue", Logging.Debug);
            return true;
        }
        ...
        private void IdleCMBState()
        {
            ...
            if (Cache.Instance.StopBot)
            {
                ...
                if (Logging.DebugAutoStart || Logging.DebugIdle)
                    Logging.Log("CombatMissionsBehavior", "DebugIdle: StopBot [" + Cache.Instance.StopBot + "]", Logging.White);
                return;
            }
            ...
            {
                if (Logging.DebugAutoStart || Logging.DebugIdle)
                    Logging.Log("CombatMissionsBehavior", "DebugIdle: InSpace [" + Cache.Instance.InSpace + "]", Logging.White);
            ...
            {
                if (Logging.DebugAutoStart || Logging.DebugIdle)
                    Logging.Log("CombatMissionsBehavior", "DebugAutoStart: Autostart [" + Settings.Instance.AutoStart + "]", Logging.White);
...
//
            ...
            if (Logging.DebugAutoStart)
                Logging.Log("CombatMissionsBehavior", "DebugIdle: Autostart is currently [" + Settings.Instance.AutoStart + "]", Logging.White);
            ...
            if (_firstStart)
            {
                if (string.IsNullOrEmpty(MissionSettings.MissionsPath) || string.IsNullOrWhiteSpace(MissionSettings.MissionsPath))
                {
                    Logging.Log("Settings",
                        "You *must* set a MissionsPath to use Questor. Without it we do not know what directory to pull mission XMLs from. Disabling Autostart.",
                        Logging.White);
...
//				ChangeCombatMissionBehaviorState(CombatMissionsBehaviorState.Switch);
        ...
        private void LocalWatchCMBState()
        {
            if (Settings.Instance.UseLocalWatch)
            {
                ...
                Logging.Log("CombatMissionsBehavior.LocalWatch",
                    "Bad standings pilots in local: We will stay 5 minutes in the station and then we will check if it is clear again", Logging.Orange);
        ...
        private void GotoBaseCMBState()
        {
                ...
                // || DateTime.UtcNow.Subtract(Time.EnteredCloseQuestor_DateTime).TotalMinutes > 10)
            {
                ...
                if (Cache.Instance.Agent.AgentId != 0)
                {
                    ...
                    {
                        Logging.Log("CombatMissionsBehavior", "Cache.Instance.Mission = Cache.Instance.GetAgentMission(AgentID); [" + exception + "]",
                            Logging.Teal);
                    }
                ...
                try
                {
                    ...
                    if (!Statistics.MissionLoggingCompleted)
                    {
                        ...
                        if (Logging.DebugStatistics)
                            Logging.Log("CombatMissionsBehavior.Idle", "Done w Statistics.WriteMissionStatistics(AgentID);", Logging.Teal);
                        return;
                    }
                }
                catch
                {
                    Logging.Log("CombatMissionsBehavior.Idle",
                        "if (Cache.Instance.ActiveShip != null && Cache.Instance.ActiveShip.GivenName.ToLower() == Settings.Instance.CombatShipName.ToLower())",
                        Logging.Teal);
                }
            }
        ...
        private void ExecuteMissionCMBState()
        {
            ...
            {
                Logging.Log("CombatMissionsBehavior",
                    "Out of Ammo! - Not enough [" + MissionSettings.CurrentDamageType + "] ammo in cargohold: MinimumCharges: [" + Combat.MinimumAmmoCharges +
                    "]", Logging.Orange);
                _States.CurrentCombatMissionBehaviorState = CombatMissionsBehaviorState.GotoBase;
            ...
            try
            {
                ...
                if (Traveler.Destination == null || Traveler.Destination.SolarSystemId != destination.LastOrDefault())
                {
                    if (Cache.Instance.AllBookmarks != null && Cache.Instance.AllBookmarks.Any()) //we might have no bookmarks...
                    {
                        ...
                        Logging.Log("CombatMissionsBehavior.Traveler",
                            "Destination: [" + Cache.Instance.DirectEve.Navigation.GetLocation(destination.Last()).Name + "]", Logging.White);
        ...
        private void GotoMissionCMBState()
        {
            try
            {
                    ...
                    // We assume that this will always work "correctly" (tm)
                {
                    ...
                    {
                        Logging.Log("CombatMissionsBehavior",
                            "Setting Destination to 1st bookmark from AgentID: " + Cache.Instance.Agent.AgentId + " with [" + nameOfBookmark + "] in the title",
                            Logging.White);
                ...
                {
                    Logging.Log("CombatMissionsBehavior.GotoMission",
                        "[" + Combat.PotentialCombatTargets.Count() + "] potentialCombatTargets found , Running combat.ProcessState", Logging.White);
                    Combat.ProcessState();
                }
...
//							_States.CurrentQuestorState = QuestorState.Idle;
                        ...
                        {
                            Logging.Log("CombatMissionsBehavior.Unloadloot",
                                "There are [ " + Cache.Instance.BookmarksByLabel(Settings.Instance.BookmarkPrefix + " ").Count +
                                " ] more salvage bookmarks left to process", Logging.White);
                            ...
                            if (Salvage.SalvageMultipleMissionsinOnePass)
                            {
                                ...
                                {
                                    Logging.Log("CombatMissionsBehavior.UnloadLoot",
                                        "The last finished after mission salvaging session was [" +
                                        DateTime.UtcNow.Subtract(Statistics.FinishedSalvaging).TotalMinutes + "] ago ", Logging.White);
                                    Logging.Log("CombatMissionsBehavior.UnloadLoot",
                                        "we are after mission salvaging again because it has been at least [" +
                                        (Time.Instance.WrecksDisappearAfter_minutes - Time.Instance.AverageTimeToCompleteAMission_minutes -
                                         Time.Instance.AverageTimetoSalvageMultipleMissions_minutes) + "] min since the last session. ", Logging.White);
                                ...
                                else //we are salvaging mission 'in one pass' and it has not been enough time since our last run... do another mission
                                {
                                    Logging.Log("CombatMissionsBehavior.UnloadLoot",
                                        ...
                                        DateTime.UtcNow.Subtract(Statistics.FinishedSalvaging).TotalMinutes + "] ago ", Logging.White);
                                    Logging.Log("CombatMissionsBehavior.UnloadLoot",
                                        "we are going to the next mission because it has not been [" +
                                        (Time.Instance.WrecksDisappearAfter_minutes - Time.Instance.AverageTimeToCompleteAMission_minutes -
                                         Time.Instance.AverageTimetoSalvageMultipleMissions_minutes) + "] min since the last session. ", Logging.White);
                                    _States.CurrentCombatMissionBehaviorState = CombatMissionsBehaviorState.Idle;
                                }
                            ...
                            {
                                Logging.Log("CombatMissionsBehavior.UnloadLoot",
                                    "The last after mission salvaging session was [" +
                                    Math.Round(DateTime.UtcNow.Subtract(Statistics.FinishedSalvaging).TotalMinutes, 0) + "min] ago ", Logging.White);
                                _States.CurrentCombatMissionBehaviorState = CombatMissionsBehaviorState.CheckBookmarkAge;
                                Statistics.StartedSalvaging = DateTime.UtcNow;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        _States.CurrentQuestorState = QuestorState.Idle;
                        Logging.Log("CombatMissionsBehavior.Unloadloot",
                            "CharacterMode: [" + Settings.Instance.CharacterMode + "], AfterMissionSalvaging: [" + Salvage.AfterMissionSalvaging +
                            "], CombatMissionsBehaviorState: [" + _States.CurrentCombatMissionBehaviorState + "]", Logging.White);
                        return;
                    }
        ...
        private void BeginAftermissionSalvagingCMBState()
        {
            ...
            if (DateTime.UtcNow > _nextBookmarkRefreshCheck)
            {
                ...
                if (Cache.Instance.InStation && (DateTime.UtcNow > _nextBookmarksrefresh))
                {
                    ...
                    Logging.Log("CombatMissionsBehavior.BeginAftermissionSalvaging",
                        "Refreshing Bookmarks Now: Next Bookmark refresh in [" + Math.Round(_nextBookmarksrefresh.Subtract(DateTime.UtcNow).TotalMinutes, 0) +
                        "min]", Logging.White);
                ...
                Logging.Log("CombatMissionsBehavior.BeginAftermissionSalvaging",
                    "Next Bookmark refresh in [" + Math.Round(_nextBookmarksrefresh.Subtract(DateTime.UtcNow).TotalMinutes, 0) + "min]", Logging.White);
            }
        ...
        private void SalvageCMBState()
        {
            ...
            if (deadlyNPC != null)
            {
                ...
                Logging.Log("CombatMissionsBehavior.Salvage", "could not be completed because of NPCs left in the mission: deleting on grid salvage bookmark",
                    Logging.White);
                ...
                Logging.Log("CombatMissionsBehavior.Salvage", "could not be completed because of NPCs left in the mission: on grid salvage bookmark not deleted",
                    Logging.Orange);
                ...
                //|| Cache.Instance.MyShipEntity.SalvagersAvailable && !Cache.Instance.Wrecks.Any())
            {
                ...
                {
                    Logging.Log("DedicatedBookmarkSalvageBehavior.Salvage",
                        "We have been salvaging this pocket for more than [" + Time.Instance.MaxSalvageMinutesPerPocket +
                        "] min - moving on - something probably went wrong here somewhere. (debugSalvage might help narrow down what)", Logging.White);
                }

                Logging.Log("CombatMissionsBehavior.Salvage",
                    "Finished salvaging the pocket. UnlootedContainers [" + Cache.Instance.UnlootedContainers.Count() + "] Wrecks [" + Cache.Instance.Wrecks +
                    "] Salvagers? [" + Cache.Instance.MyShipEntity.SalvagersAvailable + "]", Logging.White);
                Statistics.FinishedSalvaging = DateTime.UtcNow;
            ...
            if (Logging.DebugSalvage)
                Logging.Log("CombatMissionsBehavior", "salvage: we __cannot ever__ approach in salvage.cs so this section _is_ needed", Logging.White);
        ...
        private void SalageUseGateCMBState()
        {
            ...
            if (closest != null && closest.Distance < (int) Distances.DecloakRange)
            {
                ...
                if (closest.Activate())
                {
                    ...
                    Logging.Log("CombatMissionsBehavior.Salvage",
                        "Activate [" + closest.Name + "] and change States.CurrentCombatMissionBehaviorState to 'NextPocket'", Logging.White);
            ...
            else if (closest != null)
            {
                ...
                {
                    Logging.Log("CombatMissionsBehavior.Salvage",
                        "Warping to [" + closest.Name + "] which is [" + Math.Round(closest.Distance/1000, 0) + "k away]", Logging.White);
                }
            }
        ...
        private void PrepareStorylineGotoBaseCMBState()
        {
            ...
            if (NavigateOnGrid.AvoidBumpingThingsBool)
            {
                if (Logging.DebugGotobase)
                    Logging.Log("CombatMissionsBehavior", "PrepareStorylineGotoBase: if (Settings.Instance.AvoidBumpingThings)", Logging.White);
        ...
        private void GotoNearestStationCMBState()
        {
            ...
            if (station != null)
            {
                if (station.Distance > (int) Distances.WarptoDistance)
                {
                    if (station.WarpTo())
                    {
                        Logging.Log("CombatMissionsBehavior.GotoNearestStation",
                            "[" + station.Name + "] which is [" + Math.Round(station.Distance/1000, 0) + "k away]", Logging.White);
                        _States.CurrentCombatMissionBehaviorState = CombatMissionsBehaviorState.Salvage;
                        return;
                    }
                ...
                else
                {
                        ...
                        Cache.Instance.MyShipEntity.Velocity < 50)
                    {
                        ...
                        {
                            Logging.Log("CombatMissionsBehavior.GotoNearestStation",
                                "Approaching [" + station.Name + "] which is [" + Math.Round(station.Distance/1000, 0) + "k away]", Logging.White);
                        }
                    }
                }
        ...
        private void StorylineReturnToBaseCMBState()
        {
            ...
            {
                if (Logging.DebugGotobase)
                    Logging.Log("CombatMissionsBehavior", "StorylineReturnToBase: if (Settings.Instance.AvoidBumpingThings)", Logging.White);
        ...
        private bool CMBEveryPulse()
        {
            ...
            if (_States.CurrentPanicState == PanicState.Panic || _States.CurrentPanicState == PanicState.Panicking)
            {
                ...
                {
                    Logging.Log("PanicState." + _States.CurrentPanicState.ToString(),
                        "PanicStateReset [" + PanicStateReset + "] and It has been 30+ sec since our last session change", Logging.Debug);
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(Cache.Instance.CurrentAgent))
                {
                    if (Logging.DebugAgentInteractionReplyToAgent)
                        Logging.Log("CMB.ProcessState", "if (string.IsNullOrEmpty(Cache.Instance.CurrentAgent)) return;", Logging.Debug);
                    return;
                }
